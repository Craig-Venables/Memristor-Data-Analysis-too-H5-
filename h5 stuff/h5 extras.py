def filter_data_by_sample(hdf5_file, sample_name):
    with pd.HDFStore(hdf5_file, mode='r') as store:
        ...
        # List of data for all the keys that match the sample
        data_list = []

        for key in sample_keys:
            data = store[key]  # Load data for each key
            data_list.append(data)
            print(f"Loaded data for key: {key}")

        # Combine all data into a single DataFrame (if they have the same structure)
        if data_list:
...
# Example usage: Filter data for sample "D65-0.05mgml-ITO-PMMA(3%)-Gold-s5"
